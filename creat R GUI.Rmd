...
+ `tkconfigure`  adjust the parameters of a constructor widget.  Colors can be specified by name – for common colors – or by hex RGB values which are common in web programming
...
`tclArray` to store an array of Tcl variables. The usual list methods, [[ and $, and their forms for assignment are available for arrays, but values are referred to only by name, not index: